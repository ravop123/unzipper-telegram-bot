...
# ------------------ SPLIT ARCHIVE INTO MULTIPLE ZIPS ------------------
def split_archive_into_zips(archive_path: str, files_per_zip: int, password: Optional[str] = None) -> Tuple[bool, List[str], str]:
    """
    Extracts archive, splits all files into groups of files_per_zip,
    creates separate ZIP files for each group.
    Returns (success, list_of_zip_paths, error_message)
    """
    ...
    try:
        ...
        # Split into chunks
        chunks = [all_files[i:i + files_per_zip] for i in range(0, len(all_files), files_per_zip)]
        zip_paths = []
        for idx, chunk in enumerate(chunks, start=1):
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                for file_path in chunk:
                    # Preserve relative path from extraction root
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text(
        ...
        "• Send me **multiple `.zip` or `.7z` files** → merge all into one archive, or process a single archive.\n"
        "• **Single archive options:** Convert, Unzip, Branding Cleaner, **Split into multiple ZIPs** (ask for files per ZIP).\n"
        "• **Password-protected ZIP/7z** files are fully supported.\n"
        "• `/clean` – delete all messages in this chat.\n\n"
        "**Note:** RAR support removed.",
        parse_mode="Markdown",
    )
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if waiting for split number
    if context.user_data.get("waiting_for_split_number"):
        chat_id = update.effective_chat.id
        try:
            files_per_zip = int(update.message.text.strip())
            if files_per_zip < 1:
                raise ValueError
        except ValueError:
            await update.message.reply_text("❌ Please send a valid positive number (e.g., 200).")
            return
        context.user_data["waiting_for_split_number"] = False
        pending = context.user_data.pop("split_pending", None)
        if pending:
            await process_split_archive(chat_id, context, pending, files_per_zip)
        try:
            await update.message.delete()
        except:
            pass
...
async def retry_split_with_password(chat_id: int, context: ContextTypes.DEFAULT_TYPE, pending: dict, password: str):
    archive_path = pending["archive_path"]
    files_per_zip = pending["files_per_zip"]
    message_to_edit = pending["message_to_edit"]
    success, zip_paths, err = split_archive_into_zips(archive_path, files_per_zip, password)
    if not success:
        await context.bot.edit_message_text(chat_id=chat_id, message_id=message_to_edit, text=f"❌ Split failed: {err}")
        return
    # Send ZIP files
    for idx, zip_path in enumerate(zip_paths, start=1):
        with open(zip_path, "rb") as fp:
            bio = BytesIO(fp.read())
            bio.name = f"split_part{idx}.zip"
        sent_msg = await context.bot.send_document(chat_id=chat_id, document=bio, filename=bio.name)
...
async def process_split_archive(chat_id: int, context: ContextTypes.DEFAULT_TYPE, pending: dict, files_per_zip: int):
    archive_path = pending["archive_path"]
    message_to_edit = pending["message_to_edit"]
    # Try without password first
    success, zip_paths, err = split_archive_into_zips(archive_path, files_per_zip, None)
    if not success:
        if "password required" in err.lower():
            # Ask for password
            context.user_data["pending_split"] = {
                "archive_path": archive_path,
                "files_per_zip": files_per_zip,
                "message_to_edit": message_to_edit
            }
            await ask_for_password(chat_id, context, {"action": "retry_split"})
            return
        else:
            await context.bot.edit_message_text(chat_id=chat_id, message_id=message_to_edit, text=f"❌ Split failed: {err}")
            return
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_ext == ".txt":
        ...
        text = (
            ...
            f"{files_list}\n\n"
            f"✨ *Duplicates will be removed automatically.*\n\n"
        ...
        if context.user_data["merge_msg_id"]:
            try:
                await context.bot.edit_message_text(text=text, chat_id=chat_id, message_id=context.user_data["merge_msg_id"], reply_markup=reply_markup, parse_mode="Markdown")
            except:
    ...
    if context.user_data["archive_prompt_msg_id"]:
        try:
            await context.bot.edit_message_text(text=text, chat_id=chat_id, message_id=context.user_data["archive_prompt_msg_id"], reply_markup=reply_markup, parse_mode="Markdown")
        except Exception:
            new_msg = await user_msg.reply_text(text, reply_markup=reply_markup, parse_mode="Markdown")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "merge_now":
        ...
        if len(merge_files) < 2:
            ...
            return
        await query.edit_message_text(f"🔄 Processing {len(merge_files)} files...")
        file_paths = [path for _, path in merge_files]
        ...
        if total_unique == 0:
            await query.edit_message_text("❌ No unique lines found.")
            return
        chunks = split_lines_into_chunks(unique_lines, SPLIT_LINES_PER_FILE)
        num_chunks = len(chunks)
        status_msg = await query.message.reply_text(f"📦 Splitting {total_unique} lines into {num_chunks} file(s)...")
        for idx, chunk in enumerate(chunks, start=1):
        ...
        await query.edit_message_text(f"📋 **Files in queue:**\n\n{files_list}", parse_mode="Markdown")
        keyboard = [
            [
                InlineKeyboardButton("🔀 Merge All", callback_data="merge_now"),
                InlineKeyboardButton("🔀 Merge All (Splitted)", callback_data="merge_split")
    ...
    # ---------- ARCHIVE ACTIONS ----------
        ...
        if not pending:
            ...
            return
        # Take the last one as current archive
        orig_name, arch_path = pending[-1]
        # Clean up other pending archives
        ...
        if context.user_data.get("archive_prompt_msg_id"):
            context.user_data.pop("archive_prompt_msg_id", None)
        # Show action buttons (NEW: added Split into Multiple ZIPs)
        keyboard = [
            [
                InlineKeyboardButton("🔄 Convert", callback_data="convert"),
                InlineKeyboardButton("📂 Unzip", callback_data="unzip"),
                InlineKeyboardButton("🧹 Cleaner", callback_data="cleaner"),
            ],
            [InlineKeyboardButton("📦 Split into Multiple ZIPs", callback_data="split_archive")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
    ...
    elif action == "split_archive":
        archive_path = context.user_data.get("current_archive")
        if not archive_path or not os.path.exists(archive_path):
            await query.edit_message_text("❌ Archive not found. Please send again.")
            return
        # Ask for number of files per ZIP
        await query.edit_message_text(
            "📦 **How many files do you want in each ZIP?**\n"
            "Please send a number (e.g., `200`).\n\n"
            "_(The message will be deleted after you send the number)_",
            parse_mode="Markdown"
        )
        context.user_data["waiting_for_split_number"] = True
        context.user_data["split_pending"] = {
            "archive_path": archive_path,
        ...
        pending = context.user_data.get("pending_archives", [])
        if len(pending) < 2:
            await query.edit_message_text("❌ You need at least two archives to merge.")
            return
        keyboard = [
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text("🗜️ **Choose output format for merged archive:**", reply_markup=reply_markup, parse_mode="Markdown")
        return
        ...
        total = len(pending)
        text = f"📦 **Archives collected: {total}**\n\n{archive_list}\n\nWhat would you like to do?"
        keyboard = [
            [
                InlineKeyboardButton("📂 Process Single Archive", callback_data="process_single_archive"),
                InlineKeyboardButton("🔀 Merge All Archives", callback_data="merge_archives_choose_format")
        ...
        if context.user_data.get("archive_prompt_msg_id"):
            try:
                ...
                pass
            context.user_data.pop("archive_prompt_msg_id", None)
        await query.edit_message_text("❌ Archive merge cancelled.")
        await asyncio.sleep(2)
        try:
            await query.message.delete()
        except:
    ...
    # ---------- SINGLE ARCHIVE OPERATIONS (existing) ----------
    elif action == "unzip":
        archive_path = context.user_data.get("current_archive")
        if not archive_path or not os.path.exists(archive_path):
            await query.edit_message_text("❌ Archive not found. Please send again.")
        ...
        archive_type = get_archive_type(archive_path)
        if not archive_type:
            await query.message.reply_text("❌ Unsupported archive type.")
            return
        ...
        await query.edit_message_text("🔄 **Select target format:**", reply_markup=reply_markup, parse_mode="Markdown")
        return
        ...
        with tempfile.NamedTemporaryFile(suffix=output_ext, delete=False) as out_tmp:
            out_path = out_tmp.name
        success, error_msg = await convert_archive(archive_path, out_path, target, progress_msg.message_id, chat_id, context)
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text("✅ Archive ready. Choose an action:", reply_markup=reply_markup)
        return
        ...
        with tempfile.NamedTemporaryFile(suffix=output_ext, delete=False) as out_tmp:
            out_path = out_tmp.name
        success, error_msg = await clean_branding_in_archive(archive_path, out_path, progress_msg.message_id, chat_id, context)
        if success and error_msg != "PASSWORD_NEEDED":
            with open(out_path, "rb") as fp:
                bio = BytesIO(fp.read())
                cleaned_name = f"cleaned_{original_name}"
            ...
            await progress_msg.edit_text(f"❌ Cleaning failed.\n{error_msg[:200]}")
        return
...
def main():
    ...
    print("✅ Bot started with split archive feature and all password support.")
    app.run_polling()
